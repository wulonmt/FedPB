...
class CartPoleSwingUpActionScale(CartPoleSwingUpV1):
    def __init__(self, render_mode=None, scale = 1):
        super().__init__(render_mode)
        self.scale = scale
...
class CartPoleSwingUpV1WithAdjustablePole(CartPoleSwingUpV1):
    """CartPoleSwingUp with strictly positive reward and adjustable pole length."""

    def __init__(self, pole_length, render_mode=None):
        """
        Initialize the environment with adjustable pole length.
        
        Args:
            pole_length: Length of the pole (default: 0.6)
            render_mode: Rendering mode ('human' or 'rgb_array')
        """
        super().__init__(render_mode=render_mode)
        
        # Update pole parameters with custom length
        custom_pole = PoleParams(
            width=self.params.pole.width,
            length=pole_length,
            mass=0.5 * pole_length / 0.6
        )

        # Create new params with updated pole
        self.params = CartPoleSwingUpParams(
            gravity=self.params.gravity,
            forcemag=self.params.forcemag,
            deltat=self.params.deltat,
            friction=self.params.friction,
            x_threshold=self.params.x_threshold,
            cart=self.params.cart,
            pole=custom_pole
        )